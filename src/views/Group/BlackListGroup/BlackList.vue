<template>
    <div class="content-container">
      ...
      <div class="cards-container" v-if="loaded && !mobile">
        <span class="amount">
          {{ locations.length }} / {{locationStore.CurrentGroupBlackListLocations.length}}
        </span>
...
import LocationCardMobile from '@/components/LocationCardMobile.vue'
  ...
  export default {
    ...
    created(){
      ...
      computed: {
          ...mapState({locationStore: state => state.groups})
      },
      watch: {
        text(){
            this.search()
      ...
      methods: {
          ...
          search(){
            if(this.text === '' || this.text.search(/[a-z]/i) < 0 ) return this.isLocationSaved(this.locationStore.CurrentGroupBlackListLocations)
            this.locations = this.locationStore.CurrentGroupBlackListLocations.filter(filter => {
          ...
          setLocationsBlackListed(){
            this.locations = this.locationStore.CurrentGroupBlackListLocations
          },
          async removeBlackListCard(location){
            try {
              let groupID = this.groupID
                await this.removeLocationFromBlackList({location: location, groupID})
